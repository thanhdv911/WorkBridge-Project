    ...
    return (
        <section className="profile-panel min-w-0 overflow-hidden rounded-2xl">
            <div className="px-5 sm:px-6 py-5 border-b border-slate-100">
                ...
                <p className="text-slate-700 text-sm mt-1">Theo dõi trạng thái các lời mời nhận việc chính thức đã gửi cho ứng viên.</p>
            </div>

            {loading ? (
                <div className="p-10 text-center text-slate-800">Đang tải danh sách lời mời...</div>
            ) : offers.length === 0 ? (
                <div className="p-10 text-center">
                    ...
                    <p className="text-slate-700 mt-1">Phê duyệt ứng viên tại trang Đánh giá ứng viên, sau đó gửi lời mời nhận việc.</p>
                </div>
            ) : (
                    ...
                    {paginatedOffers.map(offer => (
                        <div key={offer.offerId} className="px-5 sm:px-6 py-5 grid lg:grid-cols-[minmax(0,1fr)_minmax(160px,220px)_auto] gap-4 lg:items-center">
                            <div className="min-w-0">
                                ...
                                <p className="text-sm text-slate-700 truncate">{offer.jobTitle}</p>
                                <p className="text-xs text-slate-800 truncate">{offer.position} tại {offer.branchName}</p>
                            ...
                            <div className="text-sm text-slate-800">
                                ...
                                <p className="text-xs text-slate-800">Bắt đầu: {new Date(offer.startDate).toLocaleDateString('vi-VN')}</p>
                            </div>
