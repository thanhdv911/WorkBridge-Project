...
export default function ProfileCover({ user, onEditClick, isOwnProfile = true, ratingStats }) {
  ...
  return (
    <section className="relative animate-fadeIn">
      ...
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 -mt-14 relative z-10">
        <div className="flex flex-col sm:flex-row sm:items-end gap-4">
          <div className="w-28 h-28 rounded-2xl bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white text-3xl font-black border-4 border-white shadow-xl select-none">
            {initials}
          </div>
          <div className="flex-1 pb-1">
            <h1 className="text-2xl font-black tracking-tight text-slate-800">{user?.fullName || 'Anonymous User'}</h1>
            <div className="flex items-center gap-3 mt-1 flex-wrap">
              ...
              {ratingStats?.totalReviews > 0 && (
                <div className="flex items-center gap-1.5 bg-amber-50 px-2.5 py-0.5 rounded-full border border-amber-100 shadow-sm animate-fadeIn">
                  ...
                  <span className="text-[10px] text-amber-400 font-bold ml-0.5">({ratingStats?.totalReviews || 0})</span>
                </div>
              )}
            </div>
          </div>
          {isOwnProfile ? (
            <button
              onClick={onEditClick}
              className="inline-flex items-center h-10 px-5 rounded-xl text-sm font-semibold text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 hover:border-slate-350 transition-colors gap-2 shadow-sm shrink-0"
            >
            ...
            <button
              ...
              className="inline-flex items-center h-10 px-5 rounded-xl text-sm font-semibold text-rose-500 bg-rose-50 border border-rose-100 hover:bg-rose-100 transition-colors gap-2 shadow-sm shrink-0"
            >
